...
public class UserServiceImpl implements UserService {
	...
	public Response deletenote(String token, Long noteId) 
	{
		...
		List<Collaborator> collab = note.getUserColaborators();
		for(Collaborator collabs : collab)
		{
			((List<Collaborator>)note.getUserColaborators()).remove(collabs);
		}
		...
		{
			((List<Label>)note.getLabel()).remove(labels);
			((List<Note>)labels.getNote()).remove(note);
	...
	public Response trashNote(String token, Long noteid) {
		...
		if (note.isTrashStatus() == true) {
			...
			noteRepository.save(note);
			return ResponseHelper.statusResponse(201, "Note is restore");
		}
		return statusResponse;
	}
	...
	public Response pinNote(String token, Long noteid) {
		...
		if (note.isPinStatus()) {
			note.setPinStatus(false);
			noteRepository.save(note);
			return ResponseHelper.statusResponse(201, "Note is unpinned");
		}
		return statusResponse;
	}
	...
	public Response addCollaboratorsToNote(String token, Long noteid, Collaboratordto collabDto) {
		...
		Optional<User> collabUsers = userRepo.findByEmailId(collab.getEmail());
		if (collabUsers.isPresent()) {
			...
			Note noteEntity = modelMapper.map(noteDto,Note.class);
			noteEntity.setUserid(collabUsers.get().getUserId());
			
			((List<Note>) collabUsers.get().getNotes()).add(noteEntity);
			noteRepository.save(noteEntity);
			
			Collaborator collab1 = new Collaborator();
			
			collab1.setEmail(user.get().getEmailId());
			collab1.setNoteEntityId(noteEntity.getNoteid());
			collab1.setUserEntityId(collabUsers.get().getUserId());
			
			((List<Collaborator>) collabUsers.get().getCollaborators()).add(collab1);

			((List<Collaborator>) note.getUserColaborators()).add(collab1);
			
			collaboratorRepository.save(collab1);
			userRepo.save(collabUsers.get());
			noteRepository.save(note);
			
			elasticSearch.createNote(noteEntity);
